...
export default function Admin(){
  ...
  const [filter,setFilter]=useState({date:'',min:0});
  ...
  function filtered(){
    return logs.filter(l=>{
      const logDate = new Date(l.created).toISOString().split('T')[0]; // "YYYY-MM-DD"
      const dateMatch = !filter.date || logDate === filter.date;
      const minMatch = l.netAssets >= Number(filter.min || 0);
      return dateMatch && minMatch;
    });
  }
        ...
        {filtered().map((l,i)=>(
          <div key={i} className='border p-3 rounded'>
            ...
            <div className='text-sm'>
              <strong>Net:</strong> PKR {Number(l.netAssets).toLocaleString()} | 
              <strong> Zakaat:</strong> PKR {Number(l.zakaat).toLocaleString()}
            </div>
          </div>
        ))}
      </div>
